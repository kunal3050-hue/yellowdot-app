...
**For overall platform status (architecture, security, infrastructure, production, modules, testing, scores, roadmap, technical debt), see `MASTER_PLATFORM_STATUS.md` at the repo root — that is the single source of truth this document feeds into.**

**Status: canonical.** This document is the source of truth for authentication, authorization, tenant isolation, and ownership verification across the entire backend. It was written after Milestones 2–11 of the Production Hardening program closed all 8 Critical findings (C1–C8) from the 2026-07-13 security audit, each fix following the same pattern independently before this document unified them. Milestone 12 (2026-07-14) extended the same pattern to the HR/finance/admin surface (payroll, leave, performance, family, department, designation, roles, users) and added the role-assignment guard described in the baseline below. Milestones 13–16 (2026-07-14, the High-priority phase) closed the remaining audited gaps: staff management (M13, plus a second role-escalation path via the staff-invite flow), PTM parent-booking ownership (M14, plus a pre-existing `parent.uid` field bug that had silently defeated ownership checks wherever they were attempted), the attendance/QR platform (M15, including a bespoke check for `qrConfigs` records that carry no `schoolId` of their own), and the communication module (M16, plus a client-supplied-`schoolId` injection fix). See the Security Hardening Phase Completion Report (`docs/production-ops/`) for the full before/after picture.
...
9. **Every update handler must strip `schoolId` (and any other tenant/immutable key) from the client-supplied body before merging into Firestore** — spreading `{...req.body}` into an update lets an otherwise-legitimate, same-tenant writer reassign a record into a different tenant's namespace. See `communicationService.js`'s `_stripImmutable()` (Milestone 16) for the pattern; `staffService.js`'s `IMMUTABLE_FIELDS` set is the same idea applied earlier.
10. **If a record's collection has no `schoolId` field of its own** (rare — e.g. `qrConfigs`, keyed by `centerId` with no first-class owning collection), don't skip the tenant check just because `checkTenantAccess` doesn't directly apply. Write a small bespoke existence-check against a collection that *does* carry both fields (see `qrService.js`'s `centerBelongsToSchool()`, Milestone 15) rather than leaving the endpoint unchecked.
...
| `services/qrService.js` | `centerBelongsToSchool(centerId, schoolId)` | Bespoke tenant check for records with no `schoolId` of their own (queries `users` for a matching `centerId`+`schoolId`) |
| `services/communicationService.js` | `_stripImmutable(data)` (internal) | Pattern to copy: strip `schoolId`/`id` from any update body before merging into Firestore |

*Document owner: update this file in the same commit whenever a new authorization pattern is introduced, or an existing one changes. Last updated 2026-07-14 (Milestones 2–16).*
