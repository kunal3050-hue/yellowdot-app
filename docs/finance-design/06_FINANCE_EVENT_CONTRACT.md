...
| **Idempotency Key** | **Not currently well-defined** — `entryId` is unique per call, but since `createEntry()` itself has no idempotency guard yet (documented as an open gap in `04_SERVICE_CONTRACTS.md`), a retried *call* produces a second, genuinely distinct entry with its own `entryId`, not a duplicate event for the same entry. A consumer cannot yet distinguish "the same financial fact reported twice" from "two real, separate financial facts" purely from this event. **Do not build an automated, retry-capable producer of Ledger Entries until `createEntry()`'s own idempotency gap is closed** (see that service's contract) — this event's idempotency key will be finalized alongside that fix, likely `(schoolId, sourceType, sourceId)` once those fields are enforced as required. |
