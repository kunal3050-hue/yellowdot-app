...
*Executable proof: `test/financeBillingEngineValidation.test.js` (10 tests, all passing) and its supporting fixture `test/helpers/fakeFirestore.js`. Full backend regression: 325/327 passing — the 2 failures are the same pre-existing, unrelated FCM integration scripts (`e2e-push-test.js`, `fcm-test.js`) every run this session has shown, confirmed via `git log` as untouched by any Finance Platform work.*
