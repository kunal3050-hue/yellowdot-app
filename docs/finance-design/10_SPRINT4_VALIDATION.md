...
| 13 | RBAC / `finance-refund-approval` permission | **Pass.** The real Express route tree confirms `POST /api/finance/refunds/:refundId/approve` carries the same 4-guard-plus-controller depth as every other Finance Platform route; `ROLE_PERMISSIONS` confirms only `admin`/`center_owner`/`accountant` (not `center_admin`) hold `finance-refund-approval`; `authorizeRoute()` called directly confirms it rejects a `center_admin`-shaped request and allows an `accountant`-shaped one. |
...
*Executable proof: `test/financePaymentLifecycleValidation.test.js` (16 tests, all passing) and the corrected `test/helpers/fakeFirestore.js`. Full Finance Platform suite: 166/166 passing. Full backend regression: 399/401 passing — the 2 failures are the same pre-existing, unrelated FCM integration scripts (`e2e-push-test.js`, `fcm-test.js`) every run this session has shown, confirmed via `git log` as untouched by any Finance Platform work.*
